/**
 * Northstake
 * Northstake api  Operation-level extensions used by the API: - x-roleRequired (array of strings): roles required to call the operation. - x-mfaApiScope (array of strings): MFA API scopes required when the user has MFA enrolled. - x-isProSvmFeature (boolean): when true, the operation is restricted to users whose Cosmos user document has is_pro_svm_user set to true. 
 *
 * The version of the OpenAPI document: 1.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */


export class WebhookEventValidatorWithdrawalCompletedAllOfData {
   'validator_index': number;
   'validator_pubkey': string;
   'epoch': number;
   'amount_eth': string;

    static discriminator: string | undefined = undefined;

    static attributeTypeMap: Array<{name: string, baseName: string, type: string}> = [
        {
            "name": "validator_index",
            "baseName": "validator_index",
            "type": "number"
        },
        {
            "name": "validator_pubkey",
            "baseName": "validator_pubkey",
            "type": "string"
        },
        {
            "name": "epoch",
            "baseName": "epoch",
            "type": "number"
        },
        {
            "name": "amount_eth",
            "baseName": "amount_eth",
            "type": "string"
        }    ];

    static getAttributeTypeMap() {
        return WebhookEventValidatorWithdrawalCompletedAllOfData.attributeTypeMap;
    }
}


