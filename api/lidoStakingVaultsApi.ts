...
import { SVMAutomationSettings } from '../model/sVMAutomationSettings';
import { SupportedValidatorProvidersResponse } from '../model/supportedValidatorProvidersResponse';
import { TransactionResponse } from '../model/transactionResponse';
import { UpdateSVMAutomationSettingsRequest } from '../model/updateSVMAutomationSettingsRequest';
...
export class LidoStakingVaultsApi {
   ...
   public async createLidoStakingVault (createVaultRequest: CreateVaultRequest, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ body: TransactionResponse, status: number }> {
   ...
   public async renameLidoStakingContracts (contractsId: string, renameVaultRequest: RenameVaultRequest, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ body: LidoStakingContracts, status: number }> {
    ...
    /**
     * Update the automation settings for a specific staking vault. Only available to Pro SVM users. Fields omitted from the request keep their current value. When auto_update_lazy_oracle_report is enabled, Northstake automatically submits lazy oracle report updates for the staking vault whenever a new report is available. 
     * @summary Update SVM automation settings for a staking vault
     * @param contractsId The unique identifier of the Lido staking contracts
     * @param updateSVMAutomationSettingsRequest 
     */
   ...
   public async updateSVMAutomationSettings (contractsId: string, updateSVMAutomationSettingsRequest: UpdateSVMAutomationSettingsRequest, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ body: SVMAutomationSettings, status: number }> {

        const localVarPath = this.basePath + '/lidov3/staking-vaults/{contractsId}/automation-settings'
            .replace('{' + 'contractsId' + '}', encodeURIComponent(String(contractsId)));
        ...
        // Verify required parameter 'contractsId' is not null or undefined
        if (contractsId === null || contractsId === undefined) {
            throw new Error('Required parameter "contractsId" was null or undefined when calling updateSVMAutomationSettings.');
        }
        // Verify required parameter 'updateSVMAutomationSettingsRequest' is not null or undefined
        if (updateSVMAutomationSettingsRequest === null || updateSVMAutomationSettingsRequest === undefined) {
            throw new Error('Required parameter "updateSVMAutomationSettingsRequest" was null or undefined when calling updateSVMAutomationSettings.');
        }
        ...
        let localVarRequestOptions: localVarRequest.Options = {
            ...
            useQuerystring: this._useQuerystring,
            json: true,
            body: ObjectSerializer.serialize(updateSVMAutomationSettingsRequest, "UpdateSVMAutomationSettingsRequest")
        };
        ...
        return interceptorPromise.then(() => {
            if (Object.keys(localVarFormParams).length) {
                if (localVarUseFormData) {
                 ...
                 return new Promise<{ body: SVMAutomationSettings, status: number }>((resolve, reject) => {
            localVarRequest(localVarRequestOptions, (error, response, body) => {
                if (error) {
                    console.error('API call error:', error);
                    reject(error);
                    ...
                    if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                        body = ObjectSerializer.deserialize(body, "SVMAutomationSettings");
                        resolve({ body: body, status: response.statusCode });
                    } else {
                        console.error('API response error:', response.statusCode);
                        resolve({ body: body, status: response.statusCode });
