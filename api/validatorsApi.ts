...
export class ValidatorsApi {
   ...
   public async getValidators (stakingVaultId?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ body: Array<ValidatorInfo>, status: number }> {
    ...
    /**
     * Encodes a topUpExistingValidators call on the PredepositGuarantee contract to top up an already-registered validator by pubkey. Works for all vault types (pool and dedicated). 
